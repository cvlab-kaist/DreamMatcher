...
def regiter_attention_editor_diffusers(model, editor: AttentionBase):
    ...
    def ca_forward(self, model, place_in_unet):
        self.model = model
        ...
        def forward(
            ...
            )  # batch * head, height * width, channels
            sim = (
                torch.einsum("b i d, b j d -> b i j", q, k) * self.scale
            )  # batch * head, height * width, height * width
            ...
            with torch.no_grad():
                out, self_attns_mask, self_attns_mask_gen = editor(
                    q,
                    k,
                    v,
                    sim,
                    attn,
                    is_cross,
                    self.model.pca_feats,
                    self.model.latents_list,
                    self.model.pred_x0_list,
                    place_in_unet,
                    self.heads,
                    scale=self.scale,
                )
            ...
            if self_attns_mask is not None:
                model.self_attns_mask = self_attns_mask
                model.self_attns_mask_gen = self_attns_mask_gen

            return to_out(out)

        return forward

    def register_editor(net, model, count, place_in_unet):
        for name, subnet in net.named_children():
            if net.__class__.__name__ == "Attention":  # spatial Transformer layer
                net.forward = ca_forward(net, model, place_in_unet)
                return count + 1
            elif hasattr(net, "children"):
                count = register_editor(subnet, model, count, place_in_unet)
        return count

    cross_att_count = 0
    for net_name, net in model.unet.named_children():
        if "down" in net_name:
            cross_att_count += register_editor(net, model, 0, "down")
        elif "mid" in net_name:
            cross_att_count += register_editor(net, model, 0, "mid")
        elif "up" in net_name:
            cross_att_count += register_editor(net, model, 0, "up")
